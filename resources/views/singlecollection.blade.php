...
@section('title', $ccat->meta_title)
@section('meta_keywords', $ccat->meta_keywords)
@section('meta_description', $ccat->meta_description)
@section('image', asset(''))
@section('main')
<div class="container-xxl py-5 bg-dark hero-header mb-5">
    <div class="container text-center mt-5 pt-5">
        <h1 class="display-3 text-white mb-3 animated slideInDown">@if($search) {{$search}} @else {{$ccat->name}} @endif</h1>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb justify-content-center text-uppercase">
                <li class="breadcrumb-item"><a href="/">Home</a></li>
                <li class="breadcrumb-item text-white active" aria-current="page">@if($search) {{$search}} @else {{$ccat->name}} @endif</li>
            </ol>
        </nav>
    </div>
</div>
...
<div class="container">
    <div class="text-center wow fadeInUp" data-wow-delay="0.1s">
        ...
        <h1 class="mb-5">@if($search) {{$search}} @else {{$ccat->name}} @endif</h1>
    </div>
    ...
    <div class="row g-4 mt-30">
        @forelse ($collection as $coll)
        <div class="col-lg-3 col-md-6 wow fadeInUp" data-wow-delay="0.1s">
            <a data-bs-toggle="modal" href="#model_{{$coll->id}}" role="button">
